...
A solução é obtida numericamente através de otimização (método SLSQP) seguindo os fundamentos teóricos de Roncalli (2013) sob as restrições $\sum_i w_i = 1$ e limites $w_i \in [\underline{w}, \overline{w}]$. Para comparação, o Inverse Volatility Portfolio (IVP) usa a heurística simples $w_i \propto 1/\sigma_i$, mas não garante contribuições de risco iguais.
...
Em mercados emergentes como o Brasil, eventos políticos têm impacto amplificado sobre os ativos financeiros, como apontado por Carnahan e Saiegh (2021), que analisaram a influência de eleições sobre a volatilidade dos mercados emergentes. Esse contexto adverso justifica plenamente a aplicação de metodologias de alocação que busquem eficiência mesmo em cenários instáveis, como é o caso das abordagens comparadas neste trabalho.
...
\hline
Harvey et al. (2010) & Portfolio selection com momentos superiores & Análise quantitativa & Estratégias que consideram momentos superiores superam Markowitz clássico \\
\hline
Kirby e Ostdiek (2012) & Estratégias ativas simples vs. diversificação naïve & Análise empírica & Timing simples pode melhorar Equal Weight significativamente \\
\hline
Fabozzi, Huang e Zhou (2018) & Revisão de seleção robusta de portfólios & Survey metodológico & Métodos robustos são essenciais para implementação prática de otimização \\
\hline
\end{tabular}
...
\hline
Kolm, Tutuncu e Fabozzi (2014) & 60 anos de otimização de portfólios & Revisão histórica & Desafios práticos persistem; simplicidade frequentemente supera sofisticação \\
\hline
...
\hline
Lopez de Prado (2018) & Machine Learning aplicado a finanças & Metodológico & Técnicas ML podem melhorar estratégias tradicionais de alocação \\
\hline
...
\textbf{SciPy:} biblioteca voltada para computação científica, será utilizada para implementar a otimização da carteira de Markowitz através do método SLSQP (Sequential Least Squares Programming). Esta abordagem permite a formulação de problemas de programação quadrática com restrições, muito utilizada em finanças quantitativas.
